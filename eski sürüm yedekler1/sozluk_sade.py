...
def kelime_bul(kelime):
    ...
    conn = sqlite3.connect(DB_DOSYA)
    c = conn.cursor()
    c.execute(f"SELECT * FROM sozluk WHERE kelime = ?", (kelime,))
    row = c.fetchone()
    conn.close()
    if not row:
...
def benzer_kelimeler(kelime, limit=10):
    conn = sqlite3.connect(DB_DOSYA)
    c = conn.cursor()
    c.execute("SELECT kelime FROM sozluk WHERE kelime LIKE ? LIMIT ?", (f'%{kelime[:3]}%', limit+1))
...
def kelime_ekle(kelime, esanlam, zitanlam, aciklama, ornek, etiket='', notlar=''):
    ...
    conn = sqlite3.connect(DB_DOSYA)
    c = conn.cursor()
    # Eksik sütunlar için default değer ekle
...
layout = [
    [sg.Text('Kelime:', font=FONT), sg.Input(merkez, key='-KELIME-', font=FONT, enable_events=True), 
     sg.Button('Ara', font=FONT), sg.Button('Tüm Kelimeler', key='-LISTE-', font=FONT), 
     sg.Button('Dışa Aktar (CSV)', key='-DISARI-', font=FONT), sg.Button('Toplu İçe Aktar (TDK)', key='-ICERI-', font=FONT), 
    ...
    [sg.Column([zincirli_butonlar(row[cols.index('esanlamlar')])], key='-ESANLAM_PANEL-') if 'esanlamlar' in cols else sg.Text('')],
...
def panel_guncelle(kelime):
    ...
    if 'esanlamlar' in cols: window['-ESANLAM_PANEL-'].update([zincirli_butonlar(row[cols.index('esanlamlar')])])
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        break
    # ENTER tuşuyla arama
    if event == '-KELIME-' and values['-KELIME-'].strip():
        ...
        if kelime_bul(kelime)[0]:
            ...
            if benzer:
                sg.popup('Benzer kelimeler:', '\n'.join(benzer), title='Benzer Kelimeler')
            else:
